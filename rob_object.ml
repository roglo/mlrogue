...
let is_object ch = list__mem ch list_all_obj_ch;;
...
let rec list_filter f l =
  match l with
    [] -> []
  | a :: l -> if f a then a :: list_filter f l else list_filter f l
;;
...
let redefine_in_pack g ch obj =
  ...
  g.pack <- list_remove_assoc ch g.pack; g.pack <- (ch, (nb, obj)) :: g.pack
;;

let pack_list_find = (list_find : 'a -> pack -> 'b);;
...
let anti_flamer_wand_list_in_pack =
  let rec loop list =
    function
      (ch, (nb, obj)) :: rest ->
...
let enough_anti_flamer_wand_in_pack pack =
  list__list_length (anti_flamer_wand_list_in_pack pack) >= 2
;;
          ...
          let ch_arm =
            match g.worn_armor with
              Some (ch_worn, ar_worn) ->
                ...
                | None -> failwith "rob_object__check_armor"
                end
            | None -> ch1, arm1
          in
          Some ch_arm
...
let armor_of_ch g ch =
  match try Some (list__assoc ch g.pack) with Not_found -> None with
...
let wielding_a_two_handed_sword g =
  match try Some (list__assoc g.main_sword g.pack) with Not_found -> None with
    Some (_, obj) ->
      begin match obj with
        Pweapon {we_kind = WKtwo_handed_sword} -> true
      | _ -> false
  ...
  match g.worn_armor with
    Some (ch, ar) ->
      begin match ar.ar_value with
        Some v -> v
      | None -> failwith "rob_object__worn_armor"
      end
  | None -> 0
;;
...
let will_take_not_interesting_object g ch =
  if g.hallucinated || g.was_hallucinated then false
    ...
    if list__mem ch (interesting_objects g) then false else true
  else false
;;

let move_command3 g pos1 pos2 na =
  let mov = move_between pos1 pos2 in
  if (list__mem pos2 g.garbage || g.confused ||
    ...
    Some (ch, ar) ->
      let ar =
        match ar.ar_value with
          Some v ->
            {ar_protected = ar.ar_protected;
             ar_value = Some (min 99 (v + 1))}
        | None -> ar
      in
      g.worn_armor <- Some (ch, ar);
      g.armor_cursed <- false;
      redefine_in_pack g ch (Parmor ar)
  | None -> failwith "rob_object__enchant_armor"
;;

let enchant_weapon g =
  let ch = g.main_sword in
  let we =
    let (_, obj) = list__assoc ch g.pack in
    match obj with
      Pweapon we -> we
    | _ -> failwith "rob_object__enchant_weapon"
  in
  let we =
    match we.we_value with
      Some v -> {we_kind = we.we_kind; we_value = Some (v + 1)}
    | None -> we
  in
    ...
    RSread_what ->
      if transl.is_message_nothing_appropriate message then
        begin
          ...
          tempo g 1.0;
          let (nb, obj) = list__assoc ch_scr g.pack in
...
let use_object g t message na uo =
  ...
  | UOthrow_unuseful_objects (ch_junk, step) ->
            ...
            | Some (room, None) ->
                ...
                begin let rec loop dist pos =
                  if inside_room room pos && dist < 25 then
                    ...
                    let garbage pos =
                      if list__mem pos g.garbage then ()
      ...
      | 2 ->
          let (nb, obj) = list__assoc ch_junk g.pack in
