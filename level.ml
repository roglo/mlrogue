...
let nb_traps = list__list_length trap_strings;;

let trap_string t = (list__assoc t trap_strings).t_title;;
let trap_mess t = (list__assoc t trap_strings).t_mess;;
...
let is_all_connected g =
  let rooms_visited = vect__make_vect 9 false in
  let starting_room =
    let rec loop i =
      if i = 9 then i
...
let fill_it g rn do_rec_de =
  ...
  let offsets =
    f_array.efield__get g.env "fill_it_offset" [| -1; 1; 3; -3 |]
  in
  for i = 0 to 9 do
  ...
  f_array.efield__set g.env "fill_it_offset" offsets;
  let rec loop i =
    if i >= 4 then ()
    else
...
let put_amulet g = rand_place g (object__get_amulet None);;
    ...
    begin
      ...
      let rec loop j =
        if j = 9 then ()
          ...
          if i < 9 - 1 then (let _ = connect_rooms g i (i + 1) in ());
          if i < 9 - 3 then (let _ = connect_rooms g i (i + 3) in ());
          if i < 9 - 2 then
    ...
    if i = n then nf
      ...
      let rec loop_j nf j =
        if j < 250 then
          ...
          in
          if found then
            let obj = object__gr_object g in
      ...
      if no_room_for_monster g rn then ()
        ...
        let rec loop_j j =
          if j < 250 then
            ...
            then
              let monster = imonster__gr_monster g (Some shift_lev) in
...
let plant_gold g row col is_maze =
  ...
  let obj = object__get_gold (Some q) in place_at g obj row col
;;
...
let put_objects g =
  if g.cur_level < g.max_level then ()
    ...
    for i = 0 to n - 1 do
      let obj = object__gr_object g in rand_place g obj
    done;
    put_gold g
;;
...
let add_traps g =
  ...
  for i = 0 to n - 1 do
    let tt =
      let i = get_rand 0 (nb_traps - 1) in fst (list_nth trap_strings i)
    in
    let (row, col) =
      if i = 0 then
