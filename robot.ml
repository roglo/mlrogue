...
let rec string_of_next_action g t =
  ...
  | NAgo_identify_trap (gp, step, na) ->
      sprintf "(NAgo_identify_trap e (%d,%d) t (%d,%d) '%s' <%s>" gp.epos.row
  ...
  | NAmove_in_corridor (ipos, gp, trail) ->
      sprintf "NAmove_in_corridor i=(%d,%d) e=(%d,%d) t=(%d,%d)" ipos.row
        ipos.col gp.epos.row gp.epos.col gp.tpos.row gp.tpos.col
  ...
  | NAglobal_search2 (_, _, ntimes) -> sprintf "NAglobal_search2 %d" ntimes
...
and string_of_drop_scare_state =
  ...
  | DSfree_monster (gp, mov, wallmov, ntimes) ->
      sprintf "(DSfree_monster e (%d,%d) t (%d,%d) n %d)" gp.epos.row
        gp.epos.col gp.tpos.row gp.tpos.col ntimes
  ...
  | DSgo_and_hit (gp, step) ->
      sprintf "(DSgo_and_hit e (%d,%d) t (%d,%d) %d)" gp.epos.row gp.epos.col
        gp.tpos.row gp.tpos.col step
  | DSseek_object gp ->
      sprintf "(DSseek_object e (%d,%d) t (%d,%d))" gp.epos.row gp.epos.col
        gp.tpos.row gp.tpos.col
  | DStest_move (ntest, rc) -> sprintf "(DStest_move %d)" ntest
...
and string_of_use_object =
  function
    UOeat_food (ch, step) -> sprintf "(UOeat '%c' \"%s\")" ch step
  ...
  | UOwield_sword (ch, step) -> sprintf "(UOwield_sword '%c' \"%s\")" ch step
and string_of_alone_room_state =
  ...
  | ARgo_to_door (first_door, gp) ->
      sprintf "(ARgo_to_door f %b e (%d,%d) t (%d,%d))" first_door gp.epos.row
  ...
  | ARcommand s -> sprintf "(ARcommand \"%s\")" s
...
let repetition_threshold = 100
let repetition_old_state g =
...
let home oc = fprintf oc "\027[0H"
let move oc x y = fprintf oc "\027[%d;%dH" x y
      ...
      match t.t_prev_mov with
        Some mov ->
          begin match g.rogue_room_and_door with
            Some ((rmin, cmin, rmax, cmax), Some dir) ->
              if mov = one_step_to_enter_room dir then
                let count_monsters =
                  let rec loop cnt row col =
...
let can_be_called_while_going_to_stairs =
  function
    ...
    NAseek_gold_or_monster (_, _) ->
      true
  | _ -> false

let non_interruptable_action =
  function
    NAdrop_scare_and_kill {ds_state = DSforce_kill _} | NAfight (_, _, _) |
    NAgo_in_corridor_and_hit _ | NAmove (_, _) |
    NAmove_throw2 (_, _, "direction") | NAput_ring (_, _, _) |
    NAread_scroll_for_best_armor (_, _, _) | NAtest_potions (_, 1) |
    NAuse_object (_, _) | NAwear (_, _, _) |
    NAwear_armor_and_test_scrolls (_, _, _) | NAzap (_, _, _, _) ->
      true
  | _ -> false
...
let trace_trail t trail =
  ...
  List.iter
    (fun (rt, c) ->
       ...
       if c > 1 then trace t (sprintf "x%d" c))
    room_trail;
  trace t "\n"
...
let add_object_in_pack g ch s =
  ...
  let (nb, obj) =
    ...
    if transl.is_armor s then
      ...
      let ar = {ar_value = v; ar_protected = prot} in 1, Parmor ar
    else if transl.is_scroll s then
      let scr =
        if transl.is_identified_scroll s then
          let sk = identified_scroll_kind_of_message g s in Iscroll sk
        else Uscroll s
      in
      n, Pscroll scr
      ...
      let we = {we_kind = wk; we_value = v} in n, Pweapon we
...
let play tab nrow ncol t =
  ...
  let g =
    ...
    let move_result =
      match t.t_prev_pos with
        Some old_g ->
          begin match old_g.rogue_pos, t.t_prev_comm, rogue_pos with
    ...
    match t.t_prev_game with
      Some g ->
        ...
        then
          begin
            for i = 0 to 2 do
              for j = 0 to 2 do g.visited.(i).(j) <- false done
            done;
            g.speed <- t.t_speed;
            g.time_in_level <- 0;
  ...
  begin match t.t_slow_at_level with
    Some lev -> if g.level = lev then g.speed <- 1.0
  | None -> ()
  end;
  begin match t.t_slow_at_time with
    Some time -> if g.time = time then g.speed <- 1.0
  | None -> ()
  end;
  let transl = transl g in
  if t.t_move_trace then
    begin
      begin match t.t_prev_comm with
        Some comm ->
          trace t
            (sprintf "%d (lev %d): %s '%s'%s%s%s%s%s%s%s%s%s%s%s%s <%s>\n"
               ...
               (if message <> "" then
                  sprintf " \"%s\""
                    (if t.t_no_lang_dep then "..." else message)
                else "")
      ...
      match t.t_prev_game with
        Some old_g ->
          if g.pack_full && not old_g.pack_full then trace_pack g t
      | None -> ()
    end;
  ...
  if message <> "" then
    begin
      if transl.is_message_moved_onto message then
        ...
        begin
          ...
          if transl.is_message_weaken_armor message then
            ...
            g.confused <- false
          else if transl.is_message_cosmic message then
            begin g.hallucinated <- true; g.was_hallucinated <- true end
          else if transl.is_message_boring message then
            g.hallucinated <- false
          else if transl.is_message_darkness message then
            begin g.blind <- true; g.blindness_discovered <- true end
      ...
      let prev_was_message_more =
        ...
        | None -> false
      in
      if transl.is_message_aggressive_monster message then
    ...
    begin let pos = rogue_pos g in
      match t.t_prev_pos with
        Some {rogue_pos = Some old_pos} ->
          ...
          *)
          if pos <> old_pos && g.attacked_by_flame > 0 then
            if is_at_door g old_pos then
              begin g.attacked_by_flame <- 0; g.attacked <- 0 end
            else
              begin
                g.attacked_by_flame <- g.attacked_by_flame - 1;
                if g.attacked_by_flame = 0 then g.attacked <- 0
              end
      | Some _ | None -> ()
    end;
  if transl.is_message_aggressive_monster message && not g.hallucinated &&
     g.attacked_by_flame = 0 && not g.attacked_by_invisible
  then
    begin let old_hp =
      match t.t_prev_game with
  ...
  let (comm, next_action, mov) =
    ...
    | None ->
        if g.dead || transl.is_message_dead message then
          ...
          match
            if message <> "" || non_interruptable_action t.t_next_action then
              None
            else if
          ...
          | None ->
              if message = "" && not g.confused &&
                 (match t.t_prev_comm with
                    Some (Cmov mov) ->
                      begin match t.t_prev_game with
                        Some old_g ->
              ...
              then
                ...
                | None ->
                    match
                      if message <> "" ||
                         non_interruptable_action t.t_next_action
                      then
                        None
                      else if not g.blind then None
                            ...
                            | None ->
                                match
                                  ...
                                  then
                                    ...
                                    potion_of_see_invisible_in_pack g.pack =
                                      None
                                  then
                                    None
                                  else potion_of_healing_in_pack g.pack
                                with
                                  Some (ch, _) ->
                                    let uo =
                                      UOquaff_potion (ch, QSquaff_what)
                                    in
                                    let na =
                                ...
                                | None ->
                                    match
                                      ...
                                      then
                                        ...
                                        several_potions_of_healing_in_pack
                                          g.pack
                                    with
                                      Some (ch, _) ->
                                        let uo =
                                    ...
                                    | None ->
                                        if message = "" &&
                                           ...
                                           (g.hallucination_discovered &&
                                            potion_of_extra_healing_in_pack
                                              g.pack <>
                                              None &&
                                            (g.blindness_discovered ||
                                             potion_of_healing_in_pack
                                               g.pack <>
                                               None ||
                                             potion_of_see_invisible_in_pack
                                               g.pack <>
                                               None) ||
                                            several_potions_of_extra_healing_in_pack
                                              g.pack <>
                                              None)
                                        then
                                          match
                                          ...
                                          match
                                            ...
                                            then
                                              ...
                                              potion_of_increase_strength_in_pack
                                                g.pack
                                          with
                                            Some (ch, _) ->
                                              let uo =
                                          ...
                                          | None ->
                                              match
                                                ...
                                                then
                                                  ...
                                                  scroll_of_enchant_weapon_in_pack
                                                    g.pack
                                              with
                                                Some (ch, _) ->
                                                  let uo =
                                              ...
                                              | None ->
                                                  ...
                                                  then
                                                    ...
                                                    not
                                                      (non_interruptable_action
                                                         t.t_next_action) &&
                                                    (scroll_of_remove_curse_in_pack
                                                       g.pack <>
                                                       None ||
                                                     scroll_of_enchant_armor_in_pack
                                                       g.pack <>
                                                       None ||
                                                     scroll_of_protect_armor_in_pack
                                                       g.pack <>
                                                       None)
                                                  then
                                                    let ch =
                                                  ...
                                                  then
                                                    ...
                                                    g.attacked = 0 &&
                                                    not
                                                      (can_be_called_while_going_to_stairs
                                                         t.t_next_action) &&
                                                    (repetition_old_state g ||
                                                     not g.paradise &&
                                                     all_visited g) &&
                                                    message = "" &&
                                                    g.sure_stairs_pos <>
                                                  ...
                                                  then
                                                    begin
                                                      ...
                                                      if sp <> [] then
                                                        ...
                                                        in
                                                        match
                                                          path_to_closest_static_monster
                                                            g t pos
                                                        with
                                                          Some gp ->
                                                            let na =
                                                    ...
                                                    not
                                                      (non_interruptable_action
                                                         t.t_next_action) &&
                                                    g.level >=
                                                      level_of_very_mean_monsters &&
                                                    g.map_showed_since = 0 &&
                                                    number_of_scrolls_of_magic_mapping_in_pack
                                                      g.pack >
                                                      1
                                                  then
                                                    match
                                                      scroll_of_magic_mapping_in_pack
                                                        g.pack
                                                    with
                                                      Some (ch, _) ->
                                                        let na =
                                                    ...
                                                    g.level >=
                                                      level_of_very_mean_monsters &&
                                                    not g.mon_detected &&
                                                    number_of_potions_of_detect_monsters_in_pack
                                                      g.pack >
                                                      1
                                                  then
                                                    match
                                                      potion_of_detect_monsters_in_pack
                                                        g.pack
                                                    with
                                                      Some (ch, _) ->
                                                        let na =
...
let default_monpow_fname = ".robot.monpow.txt"

let arg_breakpoint = ref (-1)
let arg_slow_at_time = ref 0
let arg_monpow_fname = ref default_monpow_fname
let arg_move_trace = ref false
let arg_no_lang_dep = ref false
let arg_slow_at_level = ref 0
let arg_stop_at_paradise = ref false
...
let make str =
  ...
  let arg_slow_at_level =
    if !arg_slow_at_level = 0 then None else Some !arg_slow_at_level
  in
  let arg_slow_at_time =
    if !arg_slow_at_time = 0 then None else Some !arg_slow_at_time
  in
  {t_prev_pos = None; t_prev_game = None; t_speed = !arg_speed;
   t_move_trace = !arg_move_trace; t_monpow_fname = !arg_monpow_fname;
   t_no_lang_dep = !arg_no_lang_dep; t_slow_at_level = arg_slow_at_level;
   t_slow_at_time = arg_slow_at_time;
   t_stop_at_paradise = !arg_stop_at_paradise; t_breakpoint = !arg_breakpoint;
