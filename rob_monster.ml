...
Grammar.extend
  (let _ : 'mon_pow_line Grammar.Entry.e = mon_pow_line in
   let grammar_entry_create s =
     Grammar.create_local_entry (Grammar.of_entry mon_pow_line) s
   in
   let armv_pow : 'armv_pow Grammar.Entry.e = grammar_entry_create "armv_pow"
...
let read_monster_power_list t =
  ...
  begin
    match (try Some (open_in monpow_fname) with
      sys__Sys_error _ -> None)
  with
    Some ic ->
      begin let rec loop () =
        match
