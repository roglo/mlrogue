...
let string_create = create_string;;
...
let end_with str s =
  ...
  sub_string str (string_length str - len) len = s
;;
...
let contains str s =
  ...
  let rec loop i j =
    if j = len then true
    else if i = string_length str then false
...
let contains_yn has has_not s =
  (has = [] || list__exists (contains s) has) &&
...
(*
   ...
   is_food = (fun s -> list__exists (contains s) list_food);
   is_identified_potion =
     (fun s ->
        list__exists (contains s)
   ...
   is_identified_wand_kind = (fun s -> list__exists (contains s) list_wand_of);
   ...
   is_message_aggressive_monster =
     (fun s ->
        list__exists (contains s) ["rate"; "touche"; "faiblir"] &&
        not
          (list__exists (contains s)
   ...
   is_message_dead = (fun s -> list__exists (contains s) list_dead);
   ...
   is_message_hold_monster =
     (fun txt -> list__exists (contains txt) ["impression d'avoir"; "gèle"]);
   is_message_less_confused =
     (fun s -> list__exists (contains s) ["sens moins"; "sentez moins"]);
   is_message_maniacal_laughter = (fun s -> contains s "rires maniaques");
   is_message_move_again = (fun s -> contains s "bouger de nouveau");
   is_message_moved_onto = (fun s -> list__exists (contains s) move_onto_list);
