      ...
      Some (room, _) ->
        dist_to_closest g room pos
          (fun ch mov ->
             ...
             not (list__mem (add_mov pos mov) g.regrets) &&
    ...
    | None ->
        ...
        then
          ...
          (monl = [] ||
           list__exists
             (fun mov ->
                not (list__mem_assoc (add_mov pos mov) g.frozen_monsters))
             monl)
        then
      ...
      if can_run_away && not bad_idea_to_run_away && jeopardized then
        ...
        | None ->
            if g.sure_stairs_pos = Some pos then
              ...
              match
                if not (list__mem pos g.garbage) && not (on_something g) then
...
let conditions_for_dropping_scare g =
  ...
  not (list__mem pos g.scare_pos) && not g.paradise &&
...
let moving_monsters_at_one_move g t pos =
  ...
  let pred path pos1 =
    ...
    not (list__exists (fun pos -> list__mem pos g.scare_pos) path)
  in
  match direct_path_excl g [] pos pred with
    Some (path, mpos) ->
      let len = list__list_length path in
    ...
    function
      k :: kl ->
        ...
        if in_dung g tpos then
          ...
          if old_can_move_to g false pos tpos && list__mem ch list_obj_ch &&
             not (list__mem tpos g.garbage)
          then
            Some mov
          else loop kl
        else loop kl
...
let active_monsters_in_room g t (rmin, cmin, rmax, cmax) =
  let rec loop row col =
    if row > rmax then false
      ...
      not (list__mem_assoc {row = row; col = col} g.frozen_monsters)
    then
      true
    else loop row (col + 1)
  in
              ...
              match g.rogue_room_and_door with
                Some (room, None) ->
                  ...
                  if dl = [] then
                    ...
                    begin match
                      begin match
                        list_find (fun (dpos, _) -> list__mem dpos g.scare_pos)
                          dl
                      with
...
let ar_return_to_base g ar gp =
  (*
    let first_door = gp.tpos = (list__hd ar.ar_doors).ard_pos in
              ...
              then
                ...
                begin
                  ...
                  match ds_opt with
                    Some ds ->
                      let ds =
                        {ds_outside_tested = false; ds_base = ds.ds_base;
              ...
              then
                ...
                with
                  Some (room, dl) ->
                    ...
                    begin match
                      list_find
                        (fun (dpos, _) -> not (list__mem dpos g.scare_pos)) dl
                    with
                      Some (dpos, _) ->
                                ...
                                then
                                  ...
                                  then
                                    ...
                                    let ds =
                                      ...
                                      {ds_nb_killed_in_corr = 0;
                                       ds_nb_attempt = n; ds_base = ds.ds_base;
                                       ds_state = ds.ds_state;
                                       ds_last_corridor_kill_time =
                                         ds.ds_last_corridor_kill_time;
                                       ds_outside_tested =
                                ...
                                with
                                  Some dist ->
                                    ...
                                    if is_monster (dung_char g.dung tpos) then
                                      if list__mem_assoc tpos g.frozen_monsters
                                      then
                                        let ds =
                                          drop_scare ds (DSgo_and_hit (gp, 1))
                                        in
      ...
      if transl.is_message_something_there message then
        ...
        begin match scroll_of_scare_monsters_in_pack g.pack with
          Some (ch, (nb, obj)) ->
            ...
            let ars =
              match
                list_find (fun ard -> not (list__mem ard.ard_pos g.scare_pos))
                  ar.ar_doors
              with
  ...
  | ARseek_object gp ->
      if message <> "" then
        ...
        if monl <> [] then
          ...
          in
          let s =
            list__it_list (fun s (len, gp) -> sprintf "%s len %d" s len)
...
let count_accessible_things_around g pos in_room =
  ...
  let rec loop nmov nmon1 nmon2 nobj di dj =
    ...
    if di = 2 then nmov, nmon1, nmon2, nobj
      ...
      if list__mem ch list_mov_ch then
      ...
      else if list__mem ch list_obj_ch then
        let (nmov, nobj) =
          if list__mem {row = row + di; col = col + dj} g.garbage then
            mov :: nmov, nobj
          else nmov, mov :: nobj
        in
        loop nmov nmon1 nmon2 nobj di (dj + 1)
                        ...
                        | None ->
                            match
                              path_to_closest2 g pos
                                (fun _ pos -> list__mem pos g.regrets)
                            with
                              Some gp ->
                                let na = NAseek_object gp in
                                Coth ` `, na, t.t_prev_mov
                    ...
                    | _ ->
                        ...
                        match monl with
                          mov :: _ ->
                            ...
                            | None ->
                                if jeopardized &&
                                   list__mem_assoc (add_mov pos mov)
                                     g.frozen_monsters
                                then
                                  match gp.path with
                                    pos1 :: path ->
  ...
  | NAzap (mov, ch, na, step) ->
      ...
      | 3 ->
          if message <> "" then
            ...
            begin
              begin match list__assoc ch g.pack with
      ...
      | None ->
          if message <> "" then
            ...
            match na with
              NAalone_in_room ar ->
                (*
                                let base = (list__hd ar.ar_doors).ard_pos in
                                let gp =
                                  let excl =
                                    let monl = monsters_around g pos in
                                    list__map (add_mov pos) monl
                                  in
