...
let f_inc_dd g interesting_objects pos2 =
  if is_trap g pos2 then 1000
    ...
    not (list__mem pos2 g.garbage) &&
    list__mem (dung_char g.dung pos2) interesting_objects
  then
    0
  else if list__mem pos2 g.garbage then 20
  else if list__mem (dung_char g.dung pos2) list_obj_ch then 20
  else 10
;;
...
let monster_path g pos tpos =
  let f_excl pos = list__mem pos g.scare_pos in
...
let direct_path_excl g excl pos pred =
  let f_excl pos = list__mem pos excl in
  let f_connected pos1 pos2 k =
    let in_room_or_at_door = current_room_possibly_at_door g pos1 <> None in
    old_can_move_to g in_room_or_at_door pos1 pos2
  in
  ...
  match gen_path g f_excl f_connected f_inc_dd pos pred with
    Some (tpos, rev_path) ->
      let path = list__tl (list__rev (tpos :: rev_path)) in Some (path, tpos)
  | None -> None
;;
...
let path_in_room_to2 g room excl pos tpos =
  match
    gen_path_in_room_to g room (fun pos -> list__mem pos excl) pos tpos
  with
    Some (tpos, rev_path) -> Some (list__tl (list__rev (tpos :: rev_path)))
  | None -> None
;;
                ...
                function
                  k :: kl ->
                    ...
                    if in_dung g npos then
                      let already =
                        list__exists
                          (fun (pos, path) -> list__mem npos (pos :: path))
                          paths_running ||
                        list__exists
                          (fun (pos, path) -> list__mem npos (pos :: path))
                          rest
                      ...
                      if not (old_can_move_to g false lpos npos) ||
                         is_inside_room g npos || list__mem npos path ||
                         already ||
                         list__exists
                           (fun (pos, path) -> list__mem npos (pos :: path))
                           paths_ended
          ...
          list__fold_left
            (fun pl (tpos, path) ->
               if tpos = ipos then pl
                 ...
                 let path = list__tl (list__rev (tpos :: path)) in
                 (path, tpos) :: pl)
            [] pl
...
let path_to_closest g graph pos =
  if nothing_to_search graph then None
    ...
    match gen_path g f_excl f_connected f_inc_dd pos pred with
      Some (tpos, rev_path) ->
        let path = list__tl (list__rev (tpos :: rev_path)) in
